...
The available features are `r names(businesses)`   
...
Using `cor(...)` we see that also here the correlation is `r cor(businesses$dist*1000, businesses$rating, use="complete")`, which is neglible.
...
The correlation is `r cor(businesses$dist*1000, businesses$review_count, use="complete")`, which again, is neglible.
